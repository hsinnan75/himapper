...
{
	int64_t gPos_end;
	int i, j, k, min_thr, max_thr, num;
	AlignmentCandidate_t AlignmentCandidate;
	vector<AlignmentCandidate_t> AlignmentVec;

	min_thr = (int)(rlen * 0.5); max_thr = (int)(rlen * 1.5); num = (int)SeedPairVec.size();
	...
	for (i = 0; i < num;)
	{
		...
		if (AlignmentCandidate.Score > min_thr && AlignmentCandidate.Score < max_thr)
		{
...
//					read.AlnReportArr[i].Score += AlignmentVec[i].SeedVec[j].rLen;
//				}
//				else
//				{
...
//					if (j == 0)
//					{
//						s = ProcessHeadSequencePair(read.seq, AlignmentVec[i].SeedVec[0]);
...
//					else if (j == num - 1)
//					{
//						s = ProcessTailSequencePair(read.seq, AlignmentVec[i].SeedVec[j]);
//						read.AlnReportArr[i].AlnScore += s;
//						if (s == 0)
//						{
